...
// ImageProc_20211546Doc.cpp: CImageProc20211546Doc 클래스의 구현
//
...
// CImageProc20211546Doc

IMPLEMENT_DYNCREATE(CImageProc20211546Doc, CDocument)

BEGIN_MESSAGE_MAP(CImageProc20211546Doc, CDocument)
END_MESSAGE_MAP()


// CImageProc20211546Doc 생성/소멸

CImageProc20211546Doc::CImageProc20211546Doc() noexcept
{
	InputImg = NULL;
	InputImg2 = NULL;
	ResultImg = NULL;
...
BOOL CImageProc20211546Doc::OnNewDocument()
{
...
// CImageProc20211546Doc serialization

void CImageProc20211546Doc::Serialize(CArchive& ar) //ar: 선택한 파일에 대한 정보 => fp 에 저장
{
	if (ar.IsStoring())
	{
...
// 축소판 그림을 지원합니다.
void CImageProc20211546Doc::OnDrawThumbnail(CDC& dc, LPRECT lprcBounds)
{
...
// 검색 처리기를 지원합니다.
void CImageProc20211546Doc::InitializeSearchContent()
{
...
void CImageProc20211546Doc::SetSearchContent(const CString& value)
{
	if (value.IsEmpty())
	{
...
// CImageProc20211546Doc 진단

#ifdef _DEBUG
void CImageProc20211546Doc::AssertValid() const
{
	CDocument::AssertValid();
}

void CImageProc20211546Doc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
void CImageProc20211546Doc::LoadImageFile(CArchive& ar)
{
	...
	{
		//bitmap file header(이미지 전반 정보) 읽기
	...
	//영상 데이터 읽기
		...
		BYTE nu[4 * 3];
		int widthfile;
		widthfile = (ImageWidth * 8 + 32) / 32 * 4; //4의 배수 만들기
...
void CImageProc20211546Doc::LoadSecondImageFile(CArchive& ar)
{
	int maxValue;
	char type[16], buf[256];
	CFile* fp = ar.GetFile();
	...
	else if (strcmp(strchr(fname, '.'), ".bmp") == 0 || strcmp(strchr(fname, '.'), ".BMP") == 0)
	{
		...
		imgw = bih.biWidth;
		imgh = bih.biHeight;
		imgd = bih.biBitCount / 8; //depth: 바이트 단위 | BITCOUNT: 비트 단위 => 8로 나눔
